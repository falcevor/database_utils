...
namespace DatabaseScriptGenerator
{
    internal class PostgreSqlGenerator : IGenerator
    {
        ...
        public string GenerateProcedureScript(ProcedureActionTypes action, GeneratorParameters parameters)
        {
            switch (action)
            {
                case ProcedureActionTypes.Add:
                    return GenerateAddProcedure(parameters.Name, parameters.TableName, 
                        parameters.Columns, parameters.Parameters);
                case ProcedureActionTypes.Modify:
                    return GenerateModifyProcedure(parameters.Name, parameters.TableName, 
                        parameters.Columns, parameters.Parameters, parameters.FilterColumns, 
                        parameters.FilterParameters);
                case ProcedureActionTypes.Remove:
                    return GenerateRemoveProcedure(parameters.Name, parameters.TableName, 
                        parameters.FilterColumns, parameters.FilterParameters);
                default:
                    return string.Empty;
            }
        }
            ...
            Parameter[] parameters, Column[] filterColumns, Parameter[] filterParameters)
        {
            ...
            if (parameters == null)
                throw new ArgumentException(nameof(parameters));
            if (columns.Length != parameters.Length) throw 
                new ArgumentException($"{nameof(columns)}'s length is not equal to {nameof(parameters)}'s length");
